...
- AI draft additions in `server/src/pcs/mcp/planning_tools.py` (`generate_plan_draft`)
- AI draft additions in `server/src/pcs/web_api/planning_routes.py` (`POST /api/projects/{project}/plans/generate-draft`)
...
- Expose `generate_plan_draft(project, goal, constraints=None, max_tasks=10)` in MCP and HTTP API.
