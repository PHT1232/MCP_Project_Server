...
| `delete_project` | `project` (required exact name or id) | `{deleted: true, id, name}`. Hard-unregisters pcs state (context, contracts/evidence, plans, index, token-savings, watcher). Does **not** delete, move, or rewrite anything on disk at `root_path` (including `.project-context/requirements.md`). The name is then reusable via `register_project`. There is no implicit default project. |
...
| `onboard` | `project=null` | One-call bootstrap for an already-registered project: `{tool_usage, briefing, ready_tasks, code_map}` — see below. |
| `add_new_project` | `name`, `root_path`, `overview` | One-call bootstrap for a brand-new project: `{project, index_status, code_map, next_steps}` — see below. |
...
`onboard` is the recommended first call for any fresh agent session on a pcs-registered project — "use the pcs onboard tool" is a complete, sufficient instruction to hand to a new agent, replacing the need for a separate onboarding prompt template. It is a pure composer with no new business logic: `tool_usage` is a fixed string stating that pcs is the exclusive tool for code search, context/"remember this" notes, and planning, and that an agent should stop and ask before silently falling back to anything else if pcs is unreachable or the project isn't registered; `briefing` is the same Markdown `get_project_briefing` returns; `ready_tasks` is the same list `list_ready_tasks(plan_id=null)` returns; `code_map` is the same top-tier graph `get_code_map(scope=null)` returns. Also available as `GET /api/projects/{project}/onboard` over plain HTTP, with an identical response shape.

`add_new_project` is the recommended first call when bootstrapping a brand-new project that is not yet registered — "use the pcs add_new_project tool" is a complete, sufficient instruction to hand to a new agent. Arguments match `register_project`: `name`, `root_path`, `overview`. It is a thin composer with no content generation: it runs the same register → index → watch → requirements-file sync sequence as `register_project`, then returns `{project, index_status, code_map, next_steps}`. `project` is the same summary `register_project` returns (`id`, `name`, `root_path`, `status_line`, both token budgets, both character limits, `expiry_policy`, `expiry_days`). `index_status` is the same payload `get_index_status` returns. `code_map` is the same top-tier graph `get_code_map` returns. `next_steps` is a fixed directive checklist telling the calling agent which pcs tools to run next (confirm indexing with `get_index_status` / `reindex`; author requirements via `get_section('requirements')` then `add_requirement` or `sync_requirements` after asking the user if none exist; `add_convention` from observed `search_code` / `get_code_map` patterns; `add_feature`; `describe_files`; `update_overview`; `set_current_focus`; then `add_decision` / `add_blocker` / `add_bug` and `create_plan_with_tasks`). pcs never invents requirements, conventions, features, diagrams, or file summaries itself — the calling agent (or the user, for requirements the agent does not know) supplies that content afterward. Also available as `POST /api/projects/add-new-project` (JSON body: `name`, `root_path`, `overview`) over plain HTTP, with an identical response shape.
...
| `search_code` | `query`, `project=null`, `scope="project"`, `subtree=null`, `files=null`, `globs=null`, `limit=20` | Ranked hits plus semantic availability/mode, `total_matches` (keyword/FTS/fuzzy pool size before any cap), and `truncated` (whether that pool was cut off — `false` does not guarantee `len(hits) == total_matches`, since hybrid re-ranking can still trim the returned list to `limit`). `limit` is `1`–`100`. |
| `retrieve_context` | `task`, `project=null`, `max_tokens=1500`, `scope="project"`, `subtree=null`, `files=null` | Token-bounded relevant code/document pack. `task` is required in every call, even with `scope="files"` — it's what results are ranked against, not a label; if you don't have a task description and just want literal/structural matches in specific files, use `search_code` instead. |
| `prepare_task` | `task=null`, `task_id=null`, `project=null`, `max_tokens=null` | Exactly one of `task` or `task_id` must be given. With `task` (free text): briefing, ≤500-token contract/close-gate, and code pack with reported budget split; null `max_tokens` uses project default, valid range `1000`–`16000`. With `task_id` (a planned task's UUID from `create_plan`/`add_plan_task`, T25/INV-PLAN-5): a bounded, role-neutral Markdown handoff prompt covering the task's objective, acceptance criteria, dependency status, and linked contract rules — never claim tokens, provider keys, or raw diffs. The prompt is written to be copy-pasted into a separate, cold-started agent/session with none of this project's own instructions loaded: it spells out `claim_task`/`heartbeat_task`/`complete_task` with the actual project/plan_id/task_id already filled in (not just a pointer to read AGENTS.md), and only mentions `record_requirement_evidence` when the task actually has a linked requirement. It also states explicitly that a `claim_task` failure (already claimed, unmet dependencies, wrong status) is a normal outcome, not a sign pcs is unreachable — the only prior guidance making that distinction (`onboard`'s tool-usage text) is returned solely by that separate bootstrap call, which a cold-started agent given just this prompt never makes. With `task_id`, raises if the task's own status is already `completed`/`cancelled` (409 over HTTP) rather than generating a "build this" prompt for finished work. With `task_id`, it also surfaces up to 2 relevant conventions and 2 relevant decisions from the project's context store (scored by linked-file/task overlap, and boosted when a decision/convention cross-references one of the task's own requirements), each with a `get_section(section=...)` drill-down pointer when truncated or capped — a section's heading is omitted entirely when nothing scores as relevant. Unused contract budget spills to code; the FR22a code floor is kept when chunks exist. |
