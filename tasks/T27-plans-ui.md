...
Build a deterministic, accessible, and reactive web UI for plans and task orchestration at `/projects/:project/plans`, supporting plan listing and detail, manual plan/task creation, dependency visualization, ready-task filtering, lease-aware claim/heartbeat/release controls, task status transitions, conflict feedback, and audit history drill-down.
...
- Provide Plan Detail view:
  - Header with plan title, goal, lifecycle status, progress bar, and lifecycle controls (`activate_plan`, `complete_plan`, `archive_plan`).
  ...
  - Filter toggle for "Ready Tasks" (showing tasks whose prerequisites are completed, unblocked, and status is `ready` or expired lease).
  - Task item card displaying: local ID, title, status badge (`draft`, `ready`, `claimed`, `in_progress`, `blocked`, `in_review`, `completed`, `cancelled`), assignee / `claimed_by`, lease expiration countdown/indicator, and linked requirement IDs.
- Task orchestration controls:
  ...
  - `set_task_status` / `complete_task`: Transitions task state using active token.
...
- Task history drill-down:
  - Modal or expandable drawer showing append-only `plan_task_events` for the selected task with timestamp, event type, actor, and payload diff.
...
- [ ] Plan list and detail views render correctly with progress counters and status badges.
...
- [ ] Task event history drawer displays chronologically ordered audit events.
...
- Vitest execution output covering routing, isolation, filters, and lease conflict states.
...
- **What was done:**
  ...
  - Built DAG visualization, ready filter, and lease-aware claim controls
  - Created task history drawer and conflict handling banners
- **Design Review:**
