...
Every acceptance criterion from `REQUIREMENTS.md` §10 is mapped to automated evidence, prior recorded live evidence, or a pending manual protocol. `verified` means an automated assertion covers the criterion; `partial` means component tests cover only part of an end-to-end criterion; `manual pending` requires an external agent, process lifecycle, physical device, or release environment.

| AC | State | Evidence |
|---|---|---|
| AC1 | verified | `test_ac1_briefing_includes_all_active_sections_under_budget` |
| AC2 | manual pending | T09 fresh-agent orientation protocol in `tasks/T09-integration.md`; requires an independent agent and observation of file reads |
...
| AC6 | partial | `test_ac6_engine_reset_preserves_context` proves DB persistence across engine recreation; a full server-process restart remains a release lifecycle check |
| AC7 | manual pending | T09 controlled token-comparison protocol in `tasks/T09-integration.md`; requires comparable independent agent runs |
...
| AC11 | partial | `test_ac11_top_level_nodes_and_edges_match_module_structure`, `test_ac11_expanding_scope_returns_only_that_subtree`, and T07 live indexed-repository run |
| AC12 | partial | `test_ac12_blocker_and_bug_flag_the_owning_nodes`, `codemap.test.ts` overlay assertions, and T07 live run |
| AC13 | partial | `test_source_returns_file_content`, `codemap.test.ts` related-entry/location assertions, and T07 live inspector run with symbols/dependencies/source |
| AC14 | partial | `test_http_app_and_ac14_http_mcp_share_the_store`, `DashboardEdit.test.tsx`, and T06 live bidirectional check |
| AC14a | partial | requirements service/MCP tests plus `requirements.test.ts`, API tests, and T06 view verification |
| AC15 | partial | `test_ac15_drop_index_schema_rebuild_preserves_context` proves schema rebuild and context preservation in-process; full restart remains a lifecycle check |
...
| AC18 | partial | `test_ac18_add_in_store_writes_a_block_into_the_file`, `test_ac18_file_status_edit_flows_to_the_store`, MCP round trip, and T06 requirements UI path |
...
Run `just accept T09` for performance, security, full tests/build, Compose lint, and the tracked-secret scan. Partial and manual-pending criteria remain explicit release limitations rather than being represented as automated passes.
