...
| AC11 | Code map top-level nodes match real modules + edges | T05 | T07 | verified (server) | `test_ac11_*`; frontend render is T07 |
| AC12 | Open blocker/bug marked on its node | T05 | T07 | verified (server) | `test_ac12_blocker_and_bug_flag_the_owning_nodes` (overlay `hot`); frontend marking is T07 |
...
| AC20 | `get_code_map` default = top-level only; expand = lazy children | T05 | T07 | verified | `test_ac20_default_is_top_tier_only_and_expansion_is_scoped` (240-file repo → ≤15 nodes, <20 KB) |
