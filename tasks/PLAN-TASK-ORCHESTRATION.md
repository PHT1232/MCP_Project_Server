...
| **D20** | **Atomic Exclusive Leases & Ephemeral Tokens:** Claiming a task atomically allocates an expiring lease with TTL and returns a cryptographically random one-time secret token. Active leases have `lease_expires_at > now()`; mutating a task with an active lease strictly requires presenting the valid token; no caller can bypass an active lease as an "operator". Expired leases (`lease_expires_at <= now()`) across claimed, in-progress, or in-review tasks can be reclaimed safely. Archiving a plan atomically revokes all active leases as a trusted administrative exception. (FR47) |
...
5. **Stale Token Rejection:**
   ...
   - If the token does not match, or lease has expired (`task.lease_expires_at <= now()`), or task was released/reclaimed/completed/cancelled/archived, the operation fails with `StaleClaimTokenError` (HTTP 409 Conflict).
...
7. **Lease Expiration Boundary Normalization:**
   - **Active/unexpired lease:** `lease_expires_at > now()`.
   - **Expired/reclaimable lease:** `lease_expires_at <= now()`.
   - **Exact boundary condition:** When `lease_expires_at == now()`, the task is strictly treated as expired and eligible for reclamation. Token presentation at `lease_expires_at == now()` is rejected as expired (`StaleClaimTokenError`).
